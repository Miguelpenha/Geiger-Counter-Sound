...
export const ThemeProvider: FC = ({ children }) => {
    ...
    async function loadTheme() {
        const themeName = await AsyncStorage.getItem('@GeigerCounterSound:theme') as (IthemeType | null)
        
        if (themeName) {
            setThemeName(themeName)
        } else {
            setThemeName(Appearance.getColorScheme() || 'light')
            AsyncStorage.setItem('@GeigerCounterSound:theme', Appearance.getColorScheme() || 'light')
        }
    }
    ...
    function mutateTheme(themeNameMutate?: IthemeType) {
        ...
        AsyncStorage.setItem('@GeigerCounterSound:theme', themeNameSelect)
